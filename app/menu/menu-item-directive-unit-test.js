...
describe('Menu item directive', function () {
    ...
    beforeEach(function () {
        ...
        //we must mock a template response. This need to duplicate the template code would be eliminated if
        //the unit test httpBackend mock implemented the ".andPassThrough" method as the e2e version does.
        $httpBackend.whenGET('menu/menu-item-template.html').respond('<a href="#/item/{{id}}">{{name}}</a>');

        //Applying the rootScope digests and refreshes the ui so our directive renders.
        $rootScope.$apply(function () {
            //We compile a directive into an angular element so we can manipulate it programmatically.
    ...
    it('should display a name and id', function () {
        ...
        expect(element.attr('href')).toBe("#/item/testId");
